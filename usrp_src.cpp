...
bool check_locked_sensor(std::vector<std::string> sensor_names,
	...
	while (true) {
		if ((not first_lock_time.is_not_a_date_time())
				and (boost::get_system_time() > ( first_lock_time	+ boost::posix_time::seconds(setup_time)))) {
...
unsigned int usrp_src::init(double rate, double freq, double gain,
		char* hostfmt, char* wirefmt) {
	...
	if (!usrp) {
		std::cout
				<< "Unable to find USRP device. Unable to issue streaming command.\n"
				<< std::endl;
		return ~0;
	}
...
//set the sample rate
	...
	std::cout
			<< boost::format("Host must support a rate of atleast %f MB/s.\n")
...
float usrp_src::streamSamples(unsigned int samples_per_recv,
		std::complex<float>* buffs) {
